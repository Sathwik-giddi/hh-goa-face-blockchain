...
2. **Search:** `src/search.py:reverse_image_search` — **two live Google Lens queries in parallel** (the detected face crop *and* the original photo — each surfaces different indexed copies of a person), plus a `google_reverse_image` fallback when the merged pool is thin. Oversized query images are auto-compressed to SerpAPI's 500KB upload cap. The merged, deduped candidate pool is re-ranked by **real face recognition with ArcFace** (InsightFace `buffalo_l` / `w600k_r50`, 512-D embeddings via onnxruntime, ~166MB auto-downloaded from the official release on first run): every candidate image is face-detected (YuNet), similarity-aligned to the 112×112 ArcFace template, and compared by cosine to the query face. Calibrated on live pairs: different people 0–16%, same person 50–100%, threshold **42.5%**. SFace remains as an offline fallback recognizer. This matches the *person*, not the picture. **Live only** — raises `RuntimeError` if `SERPAPI_API_KEY` missing. `prefer_source=reddit` tiebreak.
3. **Blockchain:**
