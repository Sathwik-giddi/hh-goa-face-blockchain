...
def _load_chain(path: Path):
    if not path.exists():
        genesis = {"index": 0, "timestamp": "2026-01-01T00:00:00+00:00", "prev_hash": "0"*64, "data_hash": hashlib.sha256(b"genesis").hexdigest(), "data": {"note": "genesis"}, "hash": ""}
        genesis["hash"] = _hash_block({k: v for k, v in genesis.items() if k != "hash"})
        return [genesis]
    with open(path) as f:
        return json.load(f)

def _save_chain(path: Path, chain):
...
def anchor_local(fingerprint: str, payload: dict, chain_file: str | Path = "chain.json") -> dict:
    ...
    return {"mode": "local", "block_index": block["index"], "block_hash": block["hash"], "prev_hash": block["prev_hash"], "data_hash": data_hash, "chain_file": str(chain_file), "explorerUrl": f"file://{chain_file.resolve()}#block-{block['index']}"}
...
def verify_local(fingerprint: str, chain_file: str | Path = "chain.json") -> dict:
    ...
    for i in range(1, len(chain)):
        if chain[i]["prev_hash"] != chain[i-1]["hash"]:
            return {"verified": False, "reason": f"chain tampered at {i}"}
    target = hashlib.sha256(fingerprint.encode()).hexdigest()
    for b in chain:
        if b["data_hash"] == target:
            return {"verified": True, "block_index": b["index"], "block_hash": b["hash"], "timestamp": b["timestamp"], "data": b["data"]}
    return {"verified": False, "reason": "fingerprint not anchored"}
